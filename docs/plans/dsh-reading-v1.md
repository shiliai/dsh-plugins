...
> 状态：M1–M2 已落地，M3/M4 部分落地（逐项见 §7.1 实施状态快照）；新增 M5「NAS 书库双向扩展」（见 issue #102、§7.2）
...
### 7.1 实施状态快照（2026-09-18，对照代码逐项核对）

| 里程碑 | 状态 | 已完成 | 未完成（欠账） |
|--------|------|--------|----------------|
| M1 阅读内核 | ✅ 完成 | 骨架 / EPUB(foliate-js) / PDF(pdf.js) / 进度持久化 / 目录 / 字号 / 主题 | — |
| M2 书源 | 🟡 大部分 | OPDS 浏览、下载代理、本地导入（后端 `/import`） | OPDS 搜索；OPDS 流式打开（Range 代理 → M5）；**本地书库 `LibraryView` 未挂载到工作台**（仅定义未渲染，导入按钮无入口） |
| M3 转换与批注 | 🔴 大部分未做 | 批注后端 CRUD、本机 `ebook-convert` 调用 | SSH 转换管道（缓存/任务制/DRM 探测 → M5）；选区工具条、批注中心前端 UI（暂缓） |
| M4 Wallabag 与沉淀 | 🟡 部分 | Wallabag 收藏/稍后读/提取/open-first、上下文注入对话 | Obsidian 导出（批注/笔记写 vault，暂缓） |
| 设置页 | 🟡 部分 | 数据源 / 根目录 / 会话切换 | 阅读偏好细项（字体族/行距/边距/快捷键/选区按钮自定义） |
| 打包发布 | ✅ 完成 | release:check、版本规范 | — |

> ⚠️ issue #79 的任务清单复选框与上述实际进度脱节，需同步勾选。

### 7.2 M5 — NAS 书库双向扩展（issue #102，2026-09 新增）

把 NAS 书库从只读扩展为双向、即开即读；并补齐「本地导入 PDF → LLM 辅助阅读 → 补元数据/概要 → 上传 calibre」的沉淀链路。决策与拆分见 #102。

**实施顺序（需求方确认）**：先做「打开 PDF（本地导入）」链路 + calibre 上传；OPDS 流式打开、Kindle 转换排其后；高亮/批注、选区问 Agent、Obsidian 导出暂缓。

| 子里程碑 | 内容 | 优先级 |
|----------|------|--------|
| M5a | **本地导入 PDF 链路打通**：把本地书库（`LibraryView` + 导入按钮）挂载到工作台左栏；本地导入 → PDF 打开 → 打开即注入对话上下文（后端 `/import`、PdfPane、`addBookContext` 均已有，补前端入口并联调） | 先做 |
| M5b | **补充元数据 + 概要总结**：书籍元数据表单（书名/作者/标签/概要），概要可借当前对话 LLM 生成后填入 | 先做 |
| M5c | **上传到 calibre**（方案 A：calibre-web `/upload`，HTTP，携带元数据/概要；凭据走 credentials refs） | 先做 |
| M5d | OPDS 流式打开（open-first，Range 代理 + 后台默认缓存）+ 保留离线下载 | 后排 |
| M5e | Kindle 转换管道收尾（SSH `ebook-convert` + 缓存 + DRM 报错 + 去占位提示） | 后排 |
| —（暂缓） | 选区工具条/选中问 Agent、高亮/批注中心 UI、Obsidian 导出、OPDS 搜索 | 暂缓 |
