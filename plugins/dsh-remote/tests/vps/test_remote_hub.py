...
class RemoteHubTests(unittest.TestCase):
    ...
    def test_admin_page_is_a_safe_read_only_operations_console(self) -> None:
        ...
        self.assertIn("<title>DSH Hub</title>", page)
        self.assertIn("Online", page)
        self.assertIn("Total instances", page)
        self.assertIn('id="refresh"', page)
        self.assertIn("refreshIntervalMs = 45_000", page)
        self.assertIn("cache: 'no-store'", page)
        self.assertIn("AbortController", page)
        self.assertIn("10_000", page)
        self.assertIn("Object.keys(item).length === 2", page)
        self.assertIn("INSTANCE_ID_RE.test(item.id)", page)
        self.assertIn("!item.id.includes('--')", page)
        self.assertIn("VALID_STATES.has(item.state)", page)
        self.assertIn("inFlight !== null", page)
    ...
    def test_monitoring_contract_has_functional_probe_alert_delivery_and_retention(self) -> None:
        ...
        self.assertIn("/sidebar/ws/terminal", health)
        self.assertIn("Upgrade: websocket", health)
        self.assertIn('protected.stdout != "401"', health)
        self.assertIn('terminal.stdout != "401"', health)
