...
class RemoteEdgeTests(unittest.TestCase):
    ...
    def test_rendered_https_site_has_socket_upgrade_offline_and_no_access_log(self) -> None:
        ...
        self.assertIn("default upgrade;", rendered)
        self.assertIn("'' '';", rendered)
        self.assertNotIn("'' close;", rendered)
        self.assertIn("keepalive 8", rendered)
        self.assertIn("access_log off", rendered)
        self.assertIn("gzip on", rendered)
        self.assertIn("gzip_comp_level 1", rendered)
        self.assertIn("gzip_min_length 1024", rendered)
        self.assertIn("gzip_proxied any", rendered)
        self.assertIn("gzip_types application/json", rendered)
        self.assertIn("gzip_vary on", rendered)
