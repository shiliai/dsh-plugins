...
import { socketCleanupArgs, socketModeArgs, sshArgs, TunnelSupervisor } from '../src/tunnel-supervisor.ts'
...
describe('TunnelSupervisor', () => {
  it('uses argument-array SSH hardening and stops after its bounded retry count', async () => {
    ...
    expect(remoteCommand).toHaveBeenNthCalledWith(1, socketCleanupArgs({ sshTarget: 'vps-tencent-tokyo', remoteSocketPath: '/var/run/dsh-remote/dsh.sock' }))
  ...
  it('reports online only after the configured stability interval survives', async () => {
    ...
    expect(remoteCommand).toHaveBeenNthCalledWith(2, socketModeArgs({ sshTarget: 'dsh@vps-tencent-tokyo', remoteSocketPath: '/home/chriswang/.local/share/dsh-remote/tunnel.sock' }))
  })

  it('kills and retries a tunnel when the remote socket cannot be secured', async () => {
    vi.useFakeTimers()
    const child = new FakeChild()
    let command = 0
    const supervisor = new TunnelSupervisor({
      ...
      stabilityDelayMs: 250,
      spawn: vi.fn(() => child as unknown as ChildProcess) as never,
      remoteCommand: vi.fn(async () => {
        command += 1
        if (command === 2) throw new Error('chmod failed')
      }),
      random: () => 0,
    })
    ...
    expect(supervisor.status()).toMatchObject({ phase: 'reconnecting', reason: 'Unable to secure remote socket.' })
  })
})
