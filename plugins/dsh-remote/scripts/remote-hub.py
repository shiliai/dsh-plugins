...
HUB_LAUNCH_SECRET_FILE_DESTINATION = f"{HUB_ROUTES_DESTINATION}/launch-secret.conf"
HUB_LAUNCH_SECRET_RE = re.compile(r"^[A-Za-z0-9_-]{43}$")
...
def launch_secret_path(args: argparse.Namespace) -> Path:
    return routes_path(args).parent / "launch-secret.conf"


def render_launch_secret_map(secret: str) -> str:
    return f"""# Generated by dsh-remote-edge. Do not edit.
# Single-use Hub launch tickets are minted by each node's dsh-remote gateway
# after it verifies this shared secret; keep the file mode at 0600.
map $host $dsh_hub_launch_secret {{
    default "{secret}";
}}
"""


def load_launch_secret(args: argparse.Namespace) -> str | None:
    path = launch_secret_path(args)
    if not path.is_file():
        return None
    if (path.stat().st_mode & 0o777) != 0o600:
        raise HubError("Hub launch secret must be mode 0600")
    secret = path.read_text(encoding="utf-8")
    for line in secret.splitlines():
        match = re.search(r'default "([^"]+)"', line)
        if match is not None:
            token = match.group(1)
            if not HUB_LAUNCH_SECRET_RE.fullmatch(token) or secret != render_launch_secret_map(token):
                raise HubError("Hub launch secret is invalid")
            return token
    raise HubError("Hub launch secret is invalid")


def ensure_launch_secret(args: argparse.Namespace) -> str:
    secret = load_launch_secret(args)
    if secret is not None:
        return secret
    edge.atomic_write(launch_secret_path(args), render_launch_secret_map(secrets.token_urlsafe(32)), 0o600)
    return load_launch_secret(args)
...
.instance-launch { justify-self: end; }
.action-status { grid-column: 1 / -1; color: #5d6975; font-size: 13px; } .action-status:empty { display: none; }
.notice { margin: 0; padding: 26px 17px; color: #5d6975; } .notice[hidden] { display: none; } .notice.error { color: #9f2727; }
@media (prefers-color-scheme: dark) { :root { background: #12191f; color: #e6edf2; } header, .section-heading { border-color: #34424d; } .metric, .instances { background: #19232b; border-color: #34424d; } li { border-color: #2b3943; } .eyebrow, .updated, .metric-label, .notice { color: #aab6bf; } .state { color: #c2ccd3; } button { border-color: #8c9aa4; } button:hover { background: #283640; } .notice.error { color: #ff9b9b; } .action-status { color: #aab6bf; } }
...
(() => {
  ...
  const LAUNCH_URL_RE = /^https:\\/\\/[a-z0-9][a-z0-9.-]{0,251}[a-z0-9]\\/#dsh-host-launch=[A-Za-z0-9_-]{43}$/;
  ...
  const openInstance = (instance, trigger, status) => {
    if (trigger.disabled) return;
    const launchWindow = window.open('', '_blank');
    if (launchWindow === null) {
      status.textContent = 'Allow pop-ups for this site to open DSH Web.';
      return;
    }
    launchWindow.opener = null;
    trigger.disabled = true;
    status.textContent = 'Creating a secure launch';
    (async () => {
      try {
        const response = await fetch(`${adminBase()}/launch/${instance.id}`, { method: 'POST', headers: { Accept: 'application/json' }, cache: 'no-store' });
        if (!response.ok) throw new Error('launch request failed');
        const payload = await response.json();
        const url = payload !== null && typeof payload === 'object' && typeof payload.url === 'string' ? payload.url : null;
        if (url === null || !LAUNCH_URL_RE.test(url)) throw new Error('invalid launch response');
        if (new URL(url).hostname !== `${instance.id}.${window.location.hostname}`) throw new Error('unexpected launch target');
        launchWindow.location.replace(url);
        status.textContent = '';
      } catch {
        launchWindow.close();
        status.textContent = `Unable to open DSH Web for ${instance.id}. Try again.`;
      } finally {
  ...
  const renderInstances = (instances) => {
    ...
    for (const instance of instances) {
      ...
      details.className = 'instance-details'; id.className = 'instance-id'; state.className = 'state'; status.className = 'action-status'; status.setAttribute('role', 'status');
      id.textContent = instance.id; state.textContent = instance.state;
      if (instance.state === 'online') {
        const launch = document.createElement('button');
        launch.type = 'button'; launch.className = 'instance-launch';
        launch.textContent = 'Open DSH Web'; launch.setAttribute('aria-label', `Open DSH Web for ${instance.id}`);
        launch.addEventListener('click', () => { openInstance(instance, launch, status); });
        row.append(details, launch, status);
      } else {
        row.append(details, status);
      }
      details.append(id, state); elements.list.append(row);
    }
  };
  let inFlight = null;
...
def render_routes(base_domain: str, instances: list[dict[str, Any]], *, https: bool, generation: str | None = None, admin_path: str | None = None) -> str:
    ...
    if https and admin_path:
        lines.append(f"include {HUB_LAUNCH_SECRET_FILE_DESTINATION};")
    lines.append("")
    ...
    if https:
        ...
        if admin_path:
            ...
            for item in sorted(instances, key=lambda value: value["id"]):
                instance_id = validate_instance_id(item["id"])
                lines.extend([
                    f"    location = {admin_path}/launch/{instance_id} {{",
                    "        auth_basic \"restricted\";",
                    f"        auth_basic_user_file {HUB_ADMIN_AUTH_DESTINATION};",
                    "        limit_req zone=dsh_hub_admin burst=5 nodelay;",
                    "        limit_req_status 429;",
                    "        add_header Cache-Control \"no-store\" always;",
                    f"        proxy_pass http://dsh_hub_{instance_id.replace('-', '_')}/__dsh_remote/hub-launch;",
                    "        proxy_http_version 1.1;",
                    "        proxy_set_header X-DSH-Hub-Launch $dsh_hub_launch_secret;",
                    "        proxy_set_header Host $host;",
...
def managed_paths(args: argparse.Namespace) -> dict[str, Path]:
    return {
        ...
        "launch_secret": launch_secret_path(args),
...
def _hub_status_locked(args: argparse.Namespace) -> dict[str, Any]:
    ...
    launch_secret_ok = True
    if admin_route(args) is not None:
        try:
            launch_secret_ok = load_launch_secret(args) is not None
        except HubError:
            launch_secret_ok = False
    configured = all([
