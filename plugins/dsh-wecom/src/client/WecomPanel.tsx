...
export function WecomPanel({ close }: { close(): void }) {
  ...
  const indicatorClass = {
    ...
    online: css.indicatorOnline,
    reconnecting: css.indicatorReconnecting,
    offline: css.indicatorOffline,
    error: css.indicatorError,
  }[phase]
  return <section className={css.panel} aria-label="WeCom connection">
    ...
    <div className={css.statusRow}><span className={`${css.indicator} ${indicatorClass}`} aria-hidden="true" /><span className={css.phase}>{phase}</span></div>
    ...
    {error !== null && <div className={css.requestError} role="alert">{error}</div>}
  </section>
}
