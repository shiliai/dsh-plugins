...
function inlineCssModules() {
  return {
    name: 'dsh-cron-inline-css-modules',
    ...
    async load(this: { addWatchFile(path: string): void }, id: string): Promise<string | null> {
      ...
      const classes = Object.fromEntries([...names].map(name => [name, `dshCron_${name}`]))
...
export default defineConfig([
  { entry: { index: 'src/index.ts' }, outDir: 'lib', format: ['esm'], platform: 'node', dts: false, clean: !DEV_HOT_LOOP, external: [/^@deepseek-ai\//], define: { __PLUGIN_VERSION__: JSON.stringify(PLUGIN_VERSION) } },
  {
